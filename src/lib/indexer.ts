...
export const useIndexerStore = create<IndexerState>(set => {
  ...
  return {
    ...
    addTransfer: transfer =>
      set(
        state => {
          const updateBalance = (balanceMap: BalanceMap, address: string, delta: bigint) => {
            ...
            if (
              ...
              throw new Error(`Invalid balance state: ${address} would have negative balance`);
            }
          ...
          updateBalance(newPerChain[transfer.chainId], transfer.from, -transfer.value);

          // Update recipient balance
          updateBalance(newAggregate, transfer.to, transfer.value);
          updateBalance(newPerChain[transfer.chainId], transfer.to, transfer.value);
...
export const createIndexer = (
  ...
  const resetChain = async (chainId: number) => {
    ...
    store.transfers
      ...
      .forEach(t => {
        ...
        if (fromBalance < t.value && t.from !== '0x0000000000000000000000000000000000000000') {
          console.warn(`Skipping invalid transfer: insufficient balance for ${t.from}`);
          return;
        }
  ...
  const backfill = async (chainId: number, fromBlock: bigint, toBlock: bigint) => {
  ...
  const initialize = async () => {
    await Promise.all(
      Object.entries(clients).map(async ([chainId, client]) => {
        const chainIdNum = Number(chainId);
        const currentBlock = await client.getBlockNumber();
        const lastProcessedBlock = useIndexerStore.getState().lastProcessedBlocks[chainIdNum];

        if (!lastProcessedBlock) {
          await backfill(chainIdNum, BigInt(0), currentBlock);
        } else if (lastProcessedBlock < currentBlock) {
          await backfill(chainIdNum, lastProcessedBlock + BigInt(1), currentBlock);
        }
      })
    );
  };
